...
Verify all logic, architecture, dependencies, security, and execution flow.
Do not modify code unless verification proves a defect.
Update the affected BWG architecture document(s), if required.
Create or update the corresponding Knowledge Base (KB) document.
Mark the repository file as VERIFIED.
Move to the next repository file.
